...
local function main()
	...
	Explorer.NewListEntry = function(index)
		...
		newEntry.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
				...
				local held = true
				local rowNode = tree[index + Explorer.Index]
				...
				task.delay(0.55, function()
					if not held or tree[index + Explorer.Index] ~= rowNode or not rowNode then return end
					selection:Set(rowNode)
					Explorer.ShowRightClick({X = startX, Y = startY})
				end)
			end

				releaseEvent = service.UserInputService.InputEnded:Connect(function(endedInput)
					if endedInput == input or (input.UserInputType == Enum.UserInputType.MouseButton1 and endedInput.UserInputType == Enum.UserInputType.MouseButton1) then
						held = false
						releaseEvent:Disconnect()
				...
				mouseEvent = service.UserInputService.InputChanged:Connect(function(input)
					if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
						...
						if dist > (input.UserInputType == Enum.UserInputType.Touch and 12 or 5) then
							held = false
							releaseEvent:Disconnect()
							mouseEvent:Disconnect()
							isRenaming = false
	...
	Explorer.Refresh = function()
		...
		for i = 1,maxNodes do
			...
			if node then
				...
				local icon = entry.Indent.Icon
				Explorer.MiscIcons:DisplayExplorerIcons(icon, obj.ClassName)
				local isAnimation = obj.ClassName == "Animation"
				icon.DexAnimationIcon.Visible = isAnimation
				if icon:FindFirstChild("IconMap") then icon.IconMap.Visible = not isAnimation end
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		context.MaxHeight = math.max(120, (camera and camera.ViewportSize.Y or 500) - 60)
		context:Clear()
		...
		context:AddRegistered("SELECT_SIBLINGS", #sList ~= 1)
		...
		if env.setclipboard then
			...
			context:AddRegistered("COPY_NAMES")
			context:AddRegistered("COPY_CLASS", #sList ~= 1)
		end
		...
		if #sList == 1 and sList[1].Obj.ClassName == "Animation" then
			context:AddRegistered("PREVIEW_ANIMATION")
		end
	...
	Explorer.InitRightClick = function()
		...
		context:Register("SELECT_SIBLINGS",{Name = "Select Siblings", OnClick = function()
			local current = selection.List[1]
			if not current or not current.Parent then return end
			local siblings = {}
			for i = 1,#current.Parent do
				local node = current.Parent[i]
				if not node.Del then siblings[#siblings+1] = node end
			end
			selection:SetTable(siblings)
			Explorer.MakeNodeVisible(current)
			Explorer.Refresh()
		end})
		...
		context:Register("COPY_NAMES",{Name = "Copy Name(s)", IconMap = Explorer.MiscIcons, Icon = "Copy", OnClick = function()
			local names = {}
			for i, node in ipairs(selection.List) do names[i] = node.Obj.Name end
			if #names > 0 then env.setclipboard(table.concat(names, "\n")) end
		end})
		context:Register("COPY_CLASS",{Name = "Copy Class", IconMap = Explorer.MiscIcons, Icon = "Copy", OnClick = function()
			if selection.List[1] then env.setclipboard(selection.List[1].Obj.ClassName) end
		end})
		context:Register("PREVIEW_ANIMATION",{Name = "Preview Animation in 3D", IconMap = Explorer.MiscIcons, Icon = "Play", OnClick = function()
			local node = selection.List[1]
			if node and node.Obj.ClassName == "Animation" then ModelViewer.ViewAnimation(node.Obj) end
		end})
	...
	Explorer.SearchFilters = { -- TODO: Use data table (so we can disable some if funcs don't exist)
		Comparison = {
			...
			["class"] = function(argString)
				local query = string.format("%q", string.lower(argString))
				return {Headers = {"local find = string.find", "local lower = string.lower"},
					Predicate = "find(lower(obj.ClassName),"..query..",1,true)"}
			end,
			["tag"] = function(argString)
				if argString == "" then return end
				return {Headers = {"local tags = game:GetService('CollectionService')"},
					Predicate = "tags:HasTag(obj,"..string.format("%q", argString)..")"}
			end,
			["attr"] = function(argString)
				if argString == "" then return end
				return {Predicate = "obj:GetAttribute("..string.format("%q", argString)..") ~= nil"}
			end,
			["path"] = function(argString)
				if argString == "" then return end
				return {Headers = {"local find = string.find", "local lower = string.lower"},
					ObjectDefs = {"local okPath, objPath = pcall(game.GetFullName,obj)"},
					Predicate = "okPath and find(lower(objPath),"..string.format("%q", string.lower(argString))..",1,true)"}
			end,
		...
		Default = function(argString,caseSensitive)
			local cleanString = string.format("%q", argString)
			if caseSensitive then
				...
				return {
					...
					Predicate = "find(lowerName," .. string.format("%q", argString:lower()) .. ",1,true)"
				}
			end
		end,
...
return search]==]
	...
	Explorer.DoSearch = function(query)
		...
		if #query > 0 then	
			...
			if Main.Elevated then
				...
				if not searchFunc then searchFunc = defaultSearch end
	...
	Explorer.InitSearch = function()
		local frame = Explorer.GuiElems.ToolBar.SearchFrame
		local searchBox = frame.SearchBox
		Explorer.GuiElems.SearchBar = searchBox
		searchBox.PlaceholderText = "Name or /class /isa /tag /attr /path"
		Lib.ViewportTextBox.convert(searchBox)

		local suggestion, pending = nil, 0
		local ghost = Instance.new("TextLabel")
		ghost.Name = "Suggestion"
		ghost.BackgroundTransparency = 1
		ghost.TextXAlignment = Enum.TextXAlignment.Left
		ghost.TextColor3 = Color3.fromRGB(128, 149, 174)
		ghost.TextTransparency = 0.2
		ghost.Font = searchBox.Font
		ghost.TextSize = searchBox.TextSize
		ghost.Size = searchBox.Size
		ghost.Position = searchBox.Position
		ghost.ZIndex = searchBox.ZIndex
		searchBox.ZIndex = ghost.ZIndex + 1
		ghost.Parent = searchBox.Parent
		...
		accept.Name = "AcceptSuggestion"
		accept.ZIndex = 3
		accept.Size = UDim2.fromOffset(18, 18)
		accept.Position = UDim2.new(1, -38, 0, 0)
		accept.BackgroundTransparency = 1
		accept.Text = ">"
		accept.TextColor3 = Color3.fromRGB(128, 178, 232)
		accept.TextSize = 16
		...
		local function complete()
			...
			searchBox.CursorPosition = #suggestion + 1
		end
		accept.MouseButton1Click:Connect(complete)
		searchBox.InputBegan:Connect(function(input)
			if input.KeyCode == Enum.KeyCode.Tab or
				(input.KeyCode == Enum.KeyCode.Right and searchBox.CursorPosition == #searchBox.Text + 1) then
				complete()
			end
		end)

		local function updateSuggestion(query)
			suggestion = nil
			if #query > 0 and #query <= 40 and not query:find("[%s/()!|&]") then
				local lowerQuery = string.lower(query)
				local best, scanned = nil, 0
				for obj, node in pairs(nodes) do
					if scanned >= 15000 then break end
					scanned = scanned + 1
					if node ~= nilNode and not node.Del then
						local name = tostring(obj)
						if #name > #query and string.sub(string.lower(name), 1, #query) == lowerQuery and
							(not best or #name < #best or (#name == #best and name < best)) then
							best = name
						end
					end
				end
			...
			ghost.Text = suggestion and (query .. suggestion:sub(#query + 1)) or ""
			accept.Visible = suggestion ~= nil
		end

		searchBox:GetPropertyChangedSignal("Text"):Connect(function()
			pending = pending + 1
			local token, query = pending, searchBox.Text
			if query == "" then
				ghost.Text = ""
				accept.Visible = false
				suggestion = nil
			...
			task.delay(0.18, function()
				if token ~= pending or not searchBox.Parent then return end
				updateSuggestion(query)
				scrollV.Index = 0
				Explorer.DoSearch(query)
			end)
		end)
		...
		frame.Reset.MouseButton1Click:Connect(Explorer.ClearSearch)
	end

	Explorer.InitKeyboardShortcuts = function()
		local inputService = service.UserInputService
		local shortcutConnection = inputService.InputBegan:Connect(function(input, consumed)
			if consumed or not Explorer.Window:IsVisible() then return end
			local code = input.KeyCode
			local searchBox = Explorer.GuiElems.SearchBar
			if code == Enum.KeyCode.F and Lib.IsCtrlDown() then
				searchBox:CaptureFocus()
				return
			end
			if inputService:GetFocusedTextBox() then return end
			if code == Enum.KeyCode.Escape and searchBox.Text ~= "" then
				Explorer.ClearSearch()
			elseif code == Enum.KeyCode.F5 then
				Explorer.DoSearch(searchBox.Text)
			elseif code == Enum.KeyCode.F2 and selection.List[1] then
				Explorer.SetRenamingNode(selection.List[1])
			elseif code == Enum.KeyCode.Up or code == Enum.KeyCode.Down then
				if #tree == 0 then return end
				local selected = selection.List[1]
				local currentIndex = selected and table.find(tree, selected) or nil
				local targetIndex = math.clamp((currentIndex or (code == Enum.KeyCode.Down and 0 or 2)) +
					(code == Enum.KeyCode.Down and 1 or -1), 1, #tree)
				local node = tree[targetIndex]
				selection:Set(node)
				selection.Piviot = node
				Explorer.ViewNode(node)
			elseif (code == Enum.KeyCode.Left or code == Enum.KeyCode.Right) and selection.List[1] then
				local node = selection.List[1]
				if code == Enum.KeyCode.Right and #node > 0 then
					expanded[node] = true
		...
		Explorer.Window.Gui.Destroying:Connect(function()
			shortcutConnection:Disconnect()
		end)
	end
	...
	Explorer.InitEntryTemplate = function()
		...
		animationIcon.Name = "DexAnimationIcon"
		animationIcon.Image = "rbxassetid://79777748023029"
		animationIcon.ImageColor3 = Color3.new(1, 1, 1)
		animationIcon.BackgroundTransparency = 1
		animationIcon.Size = UDim2.fromScale(1, 1)
		animationIcon.Visible = false
		animationIcon.Parent = entryTemplate.Indent.Icon
		...
		sys.OnDown:Connect(function(item,combo,button)
			...
			if button == 1 then
				...
				sys.IsRenaming = selection.Map[node] and not env.isonmobile
	...
	Explorer.Init = function()
		...
		Explorer.InitSearch()
		Explorer.InitKeyboardShortcuts()
...
local function main()
	...
	Lib.Window = (function()
		...
		local createGui = function(self)
			...
			windowStroke.Color = Settings.Theme.Outline2
			windowStroke.Transparency = 0.45
			windowStroke.Thickness = 1
			windowStroke.Parent = guiMain
			local barGradient = Instance.new("UIGradient")
			barGradient.Color = ColorSequence.new(Color3.fromRGB(255,255,255), Color3.fromRGB(210,220,235))
			barGradient.Rotation = 90
			barGradient.Parent = guiTopBar
		...
		static.ToggleSide = function(name)
			...
			if env.isonmobile and not side.Hidden then
				local other = side == leftSide and rightSide or leftSide
				if not other.Hidden then
					other.Hidden = true
					for _, window in ipairs(other.Windows) do window.OnDeactivate:Fire() end
				end
			end
			for i,v in pairs(side.Windows) do
				if side.Hidden then
		...
		static.SetSideVisible = function(s,vis)
			...
			side.Hidden = not vis
			if env.isonmobile and vis then
				local other = side == leftSide and rightSide or leftSide
				if not other.Hidden then
					other.Hidden = true
					for _, window in ipairs(other.Windows) do window.OnDeactivate:Fire() end
		...
		static.Init = function()
			...
			if env.isonmobile then
				...
				if viewport then
					rightSide.Width = math.min(360, math.max(210, math.floor(viewport.X * 0.86)), math.max(180, viewport.X - 20))
					leftSide.Width = rightSide.Width
				end
			end
			...
			if env.isonmobile then
				leftToggle.Size = UDim2.fromOffset(30, 40)
				rightToggle.Size = UDim2.fromOffset(30, 40)
				rightToggle.Position = UDim2.new(1, -30, 0, -40)
				leftToggle.Position = UDim2.new(0, 0, 0, -40)
			end
			...
			sidesGui:GetPropertyChangedSignal("AbsoluteSize"):Connect(function()
				if env.isonmobile then
					local width = sidesGui.AbsoluteSize.X
					local target = math.min(360, math.max(210, math.floor(width * 0.86)), math.max(180, width - 20))
					leftSide.Width = target
					rightSide.Width = target
				else
					local maxWidth = math.max(300,sidesGui.AbsoluteSize.X-static.FreeWidth)
...
local function main()
	...
	-- WorldModel hosts the cloned rig so its Animator can run inside the existing viewport.
	local worldModel, animationTrack, viewCenter
	local distance, viewRevision = 10, 0

	ModelViewer.StopViewModel = function(updating)
		if animationTrack then
			pcall(function() animationTrack:Stop(0) end)
			animationTrack = nil
		end
		if worldModel then worldModel:Destroy() worldModel = nil end
		model = nil
		viewCenter = nil
		if not updating then
			...
			if camera then camera:Destroy() camera = nil end
	...
	local function cloneForPreview(item)
		local oldArchivable = item.Archivable
		local ok, copy = pcall(function()
			item.Archivable = true
			return item:Clone()
		end)
		pcall(function() item.Archivable = oldArchivable end)
		if not ok then return nil end
		return copy
	end

	ModelViewer.ViewModel = function(item, updating)
		if not item or item == workspace or item:IsA("Terrain") or
			(not item:IsA("Model") and not item:IsA("BasePart")) then return false end
		ModelViewer.StopViewModel(updating)

		local copy = cloneForPreview(item)
		...
		viewCenter = bounds.Position
		if not updating then distance = math.clamp(math.max(size.X, size.Y, size.Z) * 1.8 + 2, 4, 100000) end
		originalModel = item
		...
		if not updating then
			...
			pathLabel.Gui.Text = "path: " .. getPath(item)
			window:Show()
			ModelViewer.IsViewing = true
			local currentRevision = viewRevision
			if ModelViewer.AutoRefresh then
				task.spawn(function()
					while ModelViewer.AutoRefresh and worldModel and originalModel == item and
						viewRevision == currentRevision do
						task.wait(1 / math.max(1, ModelViewer.RefreshRate))
						if ModelViewer.AutoRefresh and originalModel == item and viewRevision == currentRevision then
							ModelViewer.ViewModel(item, true)
						end
					end
				end)
			end
	...
	ModelViewer.ViewAnimation = function(animation)
		if not animation or animation.ClassName ~= "Animation" then return false end
		local character = plr.Character
		if not character or not character:FindFirstChildOfClass("Humanoid") then
			warn("Dex++: animation preview requires a character with a Humanoid")
			return false
		end
		ModelViewer.AutoRefresh = false
		if not ModelViewer.ViewModel(character) then
			warn("Dex++: unable to clone the character for animation preview")
			return false
		end
		local humanoid = model:FindFirstChildOfClass("Humanoid")
		if not humanoid then ModelViewer.StopViewModel() return false end
		local animator = humanoid:FindFirstChildOfClass("Animator") or Instance.new("Animator")
		animator.Parent = humanoid
		local ok, track = pcall(function() return animator:LoadAnimation(animation) end)
		if not ok or not track then
			warn("Dex++: unable to load animation in the 3D preview: " .. tostring(track))
			ModelViewer.StopViewModel()
			return false
		end
		animationTrack = track
		originalModel = animation
		window:SetTitle(animation.Name .. " - Animation Preview")
		pathLabel.Gui.Text = "path: " .. getPath(animation)
		local played, reason = pcall(function() track:Play(0.1) end)
		if not played then
			warn("Dex++: animation could not start: " .. tostring(reason))
			ModelViewer.StopViewModel()
			return false
		end
		return true
	...
	ModelViewer.Init = function()
		...
		settingsButton.Position = UDim2.new(1, -3, 0, 1)
		...
		local rotationX, rotationY = -0.2, 0
		...
		context:Register("PAUSE_ANIMATION",{Name = "Pause Animation", OnClick = function()
			if animationTrack then animationTrack:AdjustSpeed(0) end
		end})
		context:Register("RESUME_ANIMATION",{Name = "Resume Animation", OnClick = function()
			if animationTrack then animationTrack:AdjustSpeed(1) end
		end})
		context:Register("REPLAY_ANIMATION",{Name = "Replay Animation", OnClick = function()
			if animationTrack then animationTrack:Stop(0) animationTrack:Play(0.1) end
		end})
		...
		context:Register("REFRESH",{Name = "Refresh", OnClick = function()
			if originalModel then
				if originalModel.ClassName == "Animation" then
					ModelViewer.ViewAnimation(originalModel)
				else
		...
		local function ShowContext(x, y)
			context:Clear()

			context:AddRegistered("STOP", not ModelViewer.IsViewing)	
			context:AddRegistered("REFRESH", not ModelViewer.IsViewing)
			if env.setclipboard then context:AddRegistered("COPY_PATH", not ModelViewer.IsViewing) end
			context:AddRegistered("SAVE_INST", not ModelViewer.IsViewing or not env.saveinstance)
			if animationTrack then
				context:AddDivider()
				context:AddRegistered("PAUSE_ANIMATION")
				context:AddRegistered("RESUME_ANIMATION")
				context:AddRegistered("REPLAY_ANIMATION")
			end
			context:AddDivider()
			...
			if originalModel and originalModel.ClassName == "Animation" then
				-- Refreshing a cloned rig would discard its running AnimationTrack.
			elseif ModelViewer.AutoRefresh then
		...
		viewportFrame.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton2 then
				ShowContext(input.Position.X, input.Position.Y)
			elseif input.UserInputType == Enum.UserInputType.MouseButton1 and Lib.CheckMouseInGui(context.Gui) then
				HideContext()
			end
		end)
		settingsButton.MouseButton1Click:Connect(function()
			ShowContext(settingsButton.AbsolutePosition.X, settingsButton.AbsolutePosition.Y + settingsButton.AbsoluteSize.Y)
		end)
...
local function main()
	...
	Properties.Update = function()
		...
		local terms = searchText and string.split(searchText, " ") or {}
		local function matches(prop)
			for _, term in ipairs(terms) do
				if term ~= "" then
					local value = lower(prop.DisplayName or prop.Name)
					local category = lower(prop.Category or "")
					local class = lower(prop.Class or "")
					local kind = lower(prop.ValueType and prop.ValueType.Name or "")
					local key, text = term:match("^(%w+):(.*)$")
					if key == "category" then
						if not find(category, text, 1, true) then return false end
					elseif key == "type" then
						if not find(kind, text, 1, true) then return false end
					elseif key == "class" then
						if not find(class, text, 1, true) then return false end
					elseif term == "attr" then
						if not prop.IsAttribute then return false end
					elseif term == "readonly" then
						if not prop.Tags or not prop.Tags.ReadOnly then return false end
					elseif not find(value, term, 1, true) and not find(category, term, 1, true) and
						not find(class, term, 1, true) and not find(kind, term, 1, true) then
						return false
					end
				end
			end
		...
		local function recur(props,depth)
			for i = 1,#props do
				...
				local visible
				visible = (not searchText or depth ~= 1 or matches(prop))
				...
				if (visible and (expanded["CAT_"..category] or searchText)) or (prop.SpecialRow and visible) then
	...
	Properties.NewPropEntry = function(index)
		...
		nameFrame.PropName.Active = true
		nameFrame.PropName.InputBegan:Connect(function(input)
			local prop = viewList[index + Properties.Index]
			if not prop then return end
			if input.UserInputType == Enum.UserInputType.Touch then
				local beganAt = Vector2.new(input.Position.X, input.Position.Y)
				local active = true
				local ended, moved
				...
				ended = service.UserInputService.InputEnded:Connect(function(other)
					if other == input then finish() end
				end)
				moved = service.UserInputService.InputChanged:Connect(function(other)
					if other == input and (Vector2.new(other.Position.X, other.Position.Y) - beganAt).Magnitude > 12 then finish() end
				end)
				task.delay(0.55, function()
					if active and viewList[index + Properties.Index] == prop then
						Properties.DisplayPropertyContext(prop, beganAt.X, beganAt.Y)
					end
				end)
				return
			end
			if input.UserInputType == Enum.UserInputType.MouseButton2 then
				Properties.DisplayPropertyContext(prop, input.Position.X, input.Position.Y)
				return
			end
		...
		valueFrame.ValueBox.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.MouseButton2 then
				Properties.DisplayPropertyContext(viewList[index + Properties.Index], input.Position.X, input.Position.Y)
			end
		end)
	...
	Properties.DisplayPropertyContext = function(prop, x, y)
		if not prop or prop.CategoryName or not env.setclipboard then return end
		local context = Properties.CopyContext
		if not context then
			context = Lib.ContextMenu.new()
			context.Iconless = true
			context.Width = 180
			Properties.CopyContext = context
		end
		context:Clear()
		context:Add({Name = "Copy property name", OnClick = function()
			env.setclipboard(prop.Name .. (prop.SubName or ""))
		end})
		context:Add({Name = "Copy property value", OnClick = function()
			local ok, value = pcall(Properties.GetFirstPropVal, prop)
			if ok and value ~= nil then
				local converted, formatted = pcall(Properties.ValueToString, prop, value)
				env.setclipboard(converted and tostring(formatted) or tostring(value))
			end
		end})
		context:Add({Name = "Copy class name", OnClick = function()
			env.setclipboard(prop.Class)
		end})
		context:Show(x, y)
	end
	...
	Properties.InitSearch = function()
		...
		searchBox.PlaceholderText = "Name / type: / category: / attr"
		Lib.ViewportTextBox.convert(searchBox)
		...
		Properties.GuiElems.ToolBar.SearchFrame.Reset.MouseButton1Click:Connect(function()
			searchBox.Text = ""
		end)
	end
	...
	ScriptViewer.ViewScript = function(scr)
		...
		if not s or not source then
			...
			if Settings.ScriptViewer.ShowMoreInfo then
				...
				elseif not env.isdecompile or not env.isdecompile() then
...
DefaultSettings = (function()
	...
	return {
		...
		Theme = {
			...
			Main1 = rgb(43,47,54),
			Main2 = rgb(36,40,47),
			Outline1 = rgb(26,29,35), -- Mainly frames
			Outline2 = rgb(65,72,82), -- Mainly button
			Outline3 = rgb(28,34,43), -- Mainly textbox
			TextBox = rgb(29,34,42),
			Menu = rgb(31,35,42),
			ListSelection = rgb(34,100,166),
			Button = rgb(55,61,71),
			ButtonHover = rgb(69,78,91),
			ButtonPress = rgb(39,48,60),
			Highlight = rgb(77,89,103),
			Text = rgb(255,255,255),
			PlaceholderText = rgb(137,148,164),
...
Main = (function()
	...
	Main.CreateMainGui = function()
		...
		gui.OpenButton.BackgroundColor3 = Settings.Theme.Main1
		gui.OpenButton.MainFrame.BackgroundColor3 = Settings.Theme.Main1
		gui.OpenButton.MainFrame.BottomFrame.BackgroundColor3 = Settings.Theme.Main2
		gui.App.Main.BackgroundColor3 = Settings.Theme.Button
		gui.App.Main.Highlight.BackgroundColor3 = Settings.Theme.ListSelection
		local tileCorner = Instance.new("UICorner")
		tileCorner.CornerRadius = UDim.new(0, 5)
		tileCorner.Parent = gui.App.Main
