...
["ScriptViewer"] = function()
	...
	local function MainFunc()
		...
		ScriptViewer.Init = function()
			...
			-- Compact capability badge: the available function-analysis APIs are detected automatically.
			local CapabilityBadge = Instance.new("TextLabel", TopHeader)
			CapabilityBadge.Size = UDim2.new(0, 74, 1, 0)
			CapabilityBadge.BackgroundTransparency = 1
			CapabilityBadge.Font = Enum.Font.SourceSansBold
			CapabilityBadge.TextSize = 10
			CapabilityBadge.TextXAlignment = Enum.TextXAlignment.Center
			CapabilityBadge.LayoutOrder = 7
			local function RefreshCapabilityBadge()
				local getOk = type(Env.getfunctionbytecode) == "function"
				local dumpOk = type(Env.dumpbytecode) == "function" or type(Env.dumpfunctionbytecode) == "function"
				local disOk = type(Env.disassemblefunction) == "function"
				local total = (getOk and 1 or 0) + (dumpOk and 1 or 0) + (disOk and 1 or 0)
				CapabilityBadge.Text = "BYTECODE " .. tostring(total) .. "/3"
				CapabilityBadge.TextColor3 = total > 0 and Color3.fromRGB(100, 220, 140) or Color3.fromRGB(130, 130, 130)
			end
			RefreshCapabilityBadge()
			...
			local BytecodeBtn = CreateTopBtn("Bytecode", 58, 6, function()
				...
				BytecodeMenu.Size = UDim2.new(0, 198, 0, 84)
				BytecodeMenu.Position = UDim2.new(0, 220, 0, 26)
				...
				local function AddBytecodeAction(Text, Y, Callback, Enabled)
					...
					B.Font, B.TextSize, B.ZIndex = Enum.Font.SourceSans, 10, 61
					B.Active = Enabled
				...
				local function ViewFunction()
					...
					if not Text then
						...
						Text = "-- Function bytecode retrieved."
						if type(Env.decompile) == "function" then
				...
				AddBytecodeAction("View Decompiled Function", 4, ViewFunction, (HasGet or HasDump or type(Env.decompile) == "function") and Target ~= nil)
...
local func = findfunc("]] .. tostring(Target) .. [[")]]
					...
					-- Advanced function inspection. Each API is optional so the DEX
					-- remains compatible with executors that expose only some features.
					local HasGetBytecode = type(env.getfunctionbytecode) == "function"
					local HasDumpBytecode = type(env.dumpbytecode) == "function" or type(env.dumpfunctionbytecode) == "function"
					local HasDisassemble = type(env.disassemblefunction) == "function"

					if HasGetBytecode or HasDumpBytecode or HasDisassemble then
							...
							FuncContextMenu:Add({Name = "View Decompiled Function", OnClick = function()
								...
								task.spawn(function()
									...
									if not Decompiled then
										Decompiled = "-- Function bytecode was retrieved, but this executor could not decompile the returned data.\n-- Use 'Save Function Bytecode' to preserve the raw result."
									end
						...
						if HasGetBytecode or HasDumpBytecode then
							FuncContextMenu:Add({Name = "Save Function Bytecode", OnClick = function()
								local Target = DataExplorer.SelectedResult.Ref
								task.spawn(function()
									local Getter = env.getfunctionbytecode or env.dumpbytecode or env.dumpfunctionbytecode
									if type(Getter) ~= "function" then return end
									local Success, Bytecode = pcall(Getter, Target)
									if Success and Bytecode ~= nil then
										local Hash = Lib.GetHash(Target)
										local FileName = "func_" .. tostring(Hash) .. "_bytecode.txt"
										Lib.SaveAsPrompt(FileName, tostring(Bytecode))
									else
										warn("Failed to obtain function bytecode: " .. tostring(Bytecode))
									end
								end)
							end})
						end

						if HasDisassemble then
							FuncContextMenu:Add({Name = "Disassemble Function", OnClick = function()
								local Target = DataExplorer.SelectedResult.Ref
								task.spawn(function()
									local Success, ResultText = pcall(env.disassemblefunction, Target)
									if not Success or ResultText == nil then
										warn("Failed to disassemble function: " .. tostring(ResultText))
										return
									end

									local d = Instance.new("LocalScript")
									d.Name = "Disassembly_" .. ((Result.Name ~= "" and Result.Name) or "Anonymous")
									ScriptViewer.EditedScriptsCache[d] = tostring(ResultText)
									ScriptViewer.ViewScript(d)
								end)
							end})
						end
