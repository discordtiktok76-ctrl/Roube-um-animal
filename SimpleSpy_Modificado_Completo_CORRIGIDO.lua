...
Text.Position = UDim2.new(0, 31, 0, 1)
Text.Size = UDim2.new(0, 86, 0, 18)
Text.ZIndex = 2
...
RemoteIcon.Position = UDim2.new(0, 10, 0, 1)
RemoteIcon.Size = UDim2.new(0, 18, 0, 18)
RemoteIcon.Image = ""
RemoteIcon.ImageColor3 = Color3.new(1, 1, 1)
RemoteIcon.ScaleType = Enum.ScaleType.Fit
RemoteIcon.ZIndex = 3
...
-- Remote names in this table are ignored by SimpleSpy BEFORE the spy
-- builds call information, schedules a log, or creates GUI entries.
-- This does NOT prevent the remote from being used by the game.
local spyBlocklist = _G.SIMPLESPYCONFIG_BlockedRemotes
if typeof(spyBlocklist) ~= "table" then
    spyBlocklist = {}
    _G.SIMPLESPYCONFIG_BlockedRemotes = spyBlocklist
end

-- Icons used exclusively for the two Roblox remote classes.
local SIMPLESPY_REMOTE_ICONS = {
    RemoteEvent = "rbxassetid://123316292759566",
    RemoteFunction = "rbxassetid://122849223315912",
}
...
--- Ignores the specified remote from Spy processing (instance/string).
--- Unlike BlockRemote, this never prevents the actual remote call.
function SimpleSpy:BlockRemoteFromSpy(remote)
	assert(
		typeof(remote) == "Instance" or typeof(remote) == "string",
		"Instance | string expected, got " .. typeof(remote)
	)
	spyBlocklist[remote] = true
end

--- Removes a remote from the Spy processing blocklist.
function SimpleSpy:UnblockRemoteFromSpy(remote)
	assert(
		typeof(remote) == "Instance" or typeof(remote) == "string",
		"Instance | string expected, got " .. typeof(remote)
	)
	spyBlocklist[remote] = nil
end

--- Returns true when a remote is configured to be ignored by the Spy.
function SimpleSpy:IsRemoteBlockedFromSpy(remote)
	if typeof(remote) == "string" then
		return spyBlocklist[remote] == true
	end
	...
	if typeof(remote) == "Instance" then
		...
		return spyBlocklist[remote] == true or (ok and spyBlocklist[name] == true)
	end
...
function newRemote(type, name, args, remote, function_info, blocked, src, returnValue)
	...
	-- Choose the icon from the actual remote class.
	local remoteIcon = SIMPLESPY_REMOTE_ICONS.RemoteEvent
	pcall(function()
		if remote:IsA("RemoteFunction") then
			remoteIcon = SIMPLESPY_REMOTE_ICONS.RemoteFunction
		elseif remote:IsA("RemoteEvent") then
			remoteIcon = SIMPLESPY_REMOTE_ICONS.RemoteEvent
		end
	end)
	remoteFrame.RemoteIcon.Image = remoteIcon
	remoteFrame.RemoteIcon.ImageColor3 = Color3.new(1, 1, 1)
...
function remoteHandler(hookfunction, methodName, remote, args, funcInfo, calling, returnValue)
	...
	if validInstance and validClass then
		-- Hard gate: ignored remotes are not processed by the Spy.
		-- The underlying RemoteEvent/RemoteFunction is still allowed to run.
		if SimpleSpy:IsRemoteBlockedFromSpy(remote) then
			return
		end
...
function hookRemote(remoteType, remote, ...)
	if typeof(remote) == "Instance" then
		...
		if validInstance and SimpleSpy:IsRemoteBlockedFromSpy(remote) then
			-- Fully bypass Spy processing. The game call continues normally.
...
local newnamecall = newcclosure(function(remote, ...)
	if typeof(remote) == "Instance" then
		...
		if
			...
			and not SimpleSpy:IsRemoteBlockedFromSpy(remote)
			and not (blacklist[remote] or blacklist[remoteName])
		then
			local funcInfo = {}
			local calling
		...
		elseif recordReturnValues and (methodName == "InvokeServer" or methodName == "invokeServer") then
			return coroutine.yield()
		elseif
			validInstance
			and (methodName == "FireServer" or methodName == "fireServer" or methodName == "InvokeServer" or methodName == "invokeServer")
...
--- Prevents SimpleSpy from processing any remote with the selected name.
--- The remote remains fully usable by the game.
newButton("Spy Block (n)", function()
	return "Ignore every RemoteEvent/RemoteFunction with this name before SimpleSpy processes it.\nThe remote is NOT blocked from the game."
end, function()
	if selected then
		spyBlocklist[selected.Name] = true
		TextLabel.Text = "Spy block added: " .. selected.Name
	end
end)

--- Clears the dedicated Spy processing blocklist.
newButton("Clr Spy Block", function()
	return "Clear the list of remote names that SimpleSpy ignores before processing."
end, function()
	table.clear(spyBlocklist)
	TextLabel.Text = "Spy blocklist cleared!"
end)
