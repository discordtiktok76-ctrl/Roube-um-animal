...
local function main()
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		if presentClasses["Animation"] then
			...
			if #sList == 1 and sList[1].Obj:IsA("Animation") then
				context:AddRegistered("VIEW_ANIMATION_3D")
			end
		end
	...
	Explorer.InitRightClick = function()
		...
		context:Register("VIEW_ANIMATION_3D",{Name = "Animation 3D Preview", IconMap = Explorer.LegacyClassIcons, Icon = 54, OnClick = function()
			local obj = selection.List[1] and selection.List[1].Obj
			if obj and obj:IsA("Animation") then
				ModelViewer.ViewAnimation(obj)
			end
		end})
	...
	Explorer.SearchFilters = { -- TODO: Use data table (so we can disable some if funcs don't exist)
		Comparison = {
			...
			["name"] = function(argString)
				local clean = tostring(argString or "")
				if #clean == 0 then return end
				clean = clean:gsub("\\","\\\\"):gsub('"','\\"'):gsub("\n","\\n")
				return {
					Headers = {"local find = string.find"},
					ObjectDefs = {"local objName = tostring(obj.Name)"},
					Predicate = "find(string.lower(objName),\""..clean:lower().."\",1,true)"
				}
			end,
			["class"] = function(argString)
				local query = string.lower(tostring(argString or ""))
				if query == "" then return end
				return {
					Headers = {"local lower = string.lower","local find = string.find"},
					Predicate = "find(lower(obj.ClassName),\""..query:gsub('"','\\"').."\",1,true)"
				}
			end,
			["path"] = function(argString)
				...
				return {
					Headers = {"local find = string.find"},
					ObjectDefs = {"local objPath = Explorer.GetInstancePath(obj)"},
					Predicate = "find(string.lower(objPath),\""..clean:lower().."\",1,true)"
				}
			end,
			["tag"] = function(argString)
				...
				return {
					...
					Predicate = "collectionService:HasTag(obj,\""..clean.."\")"
				}
			end,
			["hasattr"] = function(argString)
				local clean = tostring(argString or "")
				if #clean == 0 then return end
				clean = clean:gsub("\\","\\\\"):gsub('"','\\"'):gsub("\n","\\n")
				return {
					Predicate = "obj:GetAttribute(\""..clean.."\") ~= nil"
				}
			end,
			...
			["tagged"] = function(argString)
				return {
					Headers = {"local collectionService = service.CollectionService"},
					Predicate = "collectionService:HasTag(obj,\""..tostring(argString or ""):gsub('"','\\"').."\")"
				}
			end,
			["type"] = function(argString)
				return Explorer.SearchFilters.Comparison["class"](argString)
			end,
			...
			["animations"] = function()
				return function()
					...
					for _, obj in ipairs(game:GetDescendants()) do
						if obj:IsA("Animation") then result[#result+1] = obj end
					end
					return result
				end
			end,
	...
	-- Advanced-search aliases kept separate so the original search syntax remains intact.
	Explorer.SearchFilters.Comparison.type = Explorer.SearchFilters.Comparison.class
...
local function main()
	...
	ModelViewer.StopAnimation = function()
		if animationTrack then
			pcall(animationTrack.Stop, animationTrack, 0.1)
			animationTrack = nil
		end
	end
