...
["ScriptViewer"] = function()
	...
	local function MainFunc()
		...
		-- Automatic function analysis pipeline.
		-- It silently selects the strongest function-analysis API available in the executor.
		local function AutoAnalyzeFunction(Target)
			if type(Target) ~= "function" then return nil, "Not a function" end

			local function tryDecompile(Value)
				if type(Env.decompile) ~= "function" then return nil end
				local ok, result = pcall(Env.decompile, Value)
				if ok and type(result) == "string" and result ~= "" then
					return result
				end
			end

			-- Direct function decompilation is preferred when supported.
			local direct = tryDecompile(Target)
			if direct then return direct, "decompile" end

			-- Prefer function-bytecode retrieval, then try the returned representation.
			local getters = {Env.getfunctionbytecode, Env.dumpfunctionbytecode, Env.dumpbytecode}
			for _, getter in ipairs(getters) do
				if type(getter) == "function" then
					local ok, bytecode = pcall(getter, Target)
					if ok and bytecode ~= nil then
						local decoded = tryDecompile(bytecode)
						if decoded then return decoded, "bytecode" end
						if type(bytecode) == "string" and bytecode ~= "" then
							return bytecode, "bytecode-raw"
						end
					end
				end
			end

			-- Disassembly is the final automatic fallback.
			if type(Env.disassemblefunction) == "function" then
				local ok, result = pcall(Env.disassemblefunction, Target)
				if ok and result ~= nil and tostring(result) ~= "" then
					return tostring(result), "disassembly"
				end
			end

			return nil, "No compatible function-analysis API"
		end
		ScriptViewer.AutoAnalyzeFunction = AutoAnalyzeFunction
		...
		ScriptViewer.Init = function()
			...
			TopHeader.Size, TopHeader.BackgroundColor3, TopHeader.BorderSizePixel = UDim2.new(1, 0, 0, 24), Settings.Theme.Main2, 0
			...
			local function CreateTopBtn(Text, Width, Order, OnClick)
				...
				Btn.TextColor3, Btn.Font, Btn.TextSize, Btn.LayoutOrder = Settings.Theme.Text, Enum.Font.SourceSans, 11, Order
			...
			-- Bytecode analysis is automatic. No separate bytecode action buttons are shown.

			ScriptViewer.IsEditing = false
			local EditBtn
			EditBtn = CreateTopBtn("Edit", 42, 8, function()
			...
			Spacer.BackgroundTransparency, Spacer.Size, Spacer.LayoutOrder = 1, UDim2.new(1, -392, 1, 0), 9

			local SearchToggleBtn = Instance.new("ImageButton", TopHeader)
			SearchToggleBtn.Size, SearchToggleBtn.BackgroundTransparency, SearchToggleBtn.LayoutOrder = UDim2.new(0, 20, 0, 20), 1, 10
			...
			CodeFrame.Frame.Position, CodeFrame.Frame.Size = UDim2.new(0, 0, 0, 24), UDim2.new(1, 0, 1, -42)
			...
			StatusFooter.Size, StatusFooter.Position, StatusFooter.BackgroundColor3, StatusFooter.BorderSizePixel = UDim2.new(1, 0, 0, 18), UDim2.new(0, 0, 1, -18), Settings.Theme.Main2, 0
			...
			InfoBar.Font, InfoBar.TextSize, InfoBar.TextColor3 = Enum.Font.SourceSans, 11, Color3.fromRGB(180, 180, 180)
		...
		SearchOverlay.Size, SearchOverlay.Position = UDim2.new(0, 210, 0, 48), UDim2.new(1, -216, 0, 28)
		SearchOverlay.BackgroundColor3, SearchOverlay.BorderColor3, SearchOverlay.BorderSizePixel, SearchOverlay.ZIndex, SearchOverlay.Visible = Color3.fromRGB(28, 28, 32), Color3.fromRGB(55, 55, 62), 1, 40, false
		...
		SearchInput.BackgroundColor3, SearchInput.BorderColor3, SearchInput.TextColor3 = Color3.fromRGB(22, 22, 25), Color3.fromRGB(50, 50, 58), Color3.fromRGB(235, 235, 240)
...
local func = findfunc("]] .. tostring(Target) .. [[")]]
					...
					-- Automatic function analysis: one action, no separate bytecode controls.
					FuncContextMenu:AddDivider()
					FuncContextMenu:Add({Name = "View Function (Auto)", OnClick = function()
						local Target = DataExplorer.SelectedResult.Ref
						task.spawn(function()
							local Text, Method = ScriptViewer.AutoAnalyzeFunction(Target)
							if not Text then
								warn("Automatic function analysis failed: " .. tostring(Method))
								return
							end
							local d = Instance.new("LocalScript")
							d.Name = "AutoFunction_" .. ((Result.Name ~= "" and Result.Name) or "Anonymous")
							ScriptViewer.EditedScriptsCache[d] = "-- Automatic analysis mode: " .. tostring(Method) .. "\\n\\n" .. Text
...
DefaultSettings = (function()
	...
	return {
		...
		Theme = {
			...
			Main1 = rgb(24,24,28),
			Main2 = rgb(30,30,35),
			Outline1 = rgb(52,52,60),
			Outline2 = rgb(60,60,68),
			Outline3 = rgb(45,45,52),
			TextBox = rgb(22,22,26),
			Menu = rgb(20,20,24),
			ListSelection = rgb(11,90,175),
			Button = rgb(42,42,48),
			ButtonHover = rgb(55,55,63),
			ButtonPress = rgb(34,34,40),
			Highlight = rgb(64,64,72),
...
Main = (function()
	...
	Main.InitEnv = function()
		...
		-- Optional function-bytecode/disassembly APIs.
		env.dumpbytecode = missing("function", dumpbytecode)
		env.dumpfunctionbytecode = missing("function", dumpfunctionbytecode)
		env.disassemblefunction = missing("function", disassemblefunction)
...
cloneref(game:GetService("WebViewService")):CloseWindow()
		...
		-- Click Select melhorado: seleciona o alvo clicado e destaca automaticamente
		-- outros objetos estruturalmente iguais (mesmo nome/classe/estrutura).
		local cptsOnMouseClick = nil
		local clickSelectHighlights = {}
		local CLICK_SELECT_MAX_MATCHES = 500

		local function clearClickSelectHighlights()
			for i = #clickSelectHighlights, 1, -1 do
				local h = clickSelectHighlights[i]
				if h then pcall(function() h:Destroy() end) end
				clickSelectHighlights[i] = nil
			end
		end

		-- Cria uma assinatura estável do objeto sem considerar posição, Parent ou
		-- propriedades que normalmente variam entre cópias (CFrame/posição).
		local function getClickSelectSignature(obj)
			if not obj then return nil end
			local parts = {obj.ClassName, obj.Name}

			if obj:IsA("BasePart") then
				parts[#parts + 1] = "Size=" .. tostring(obj.Size)
				parts[#parts + 1] = "Shape=" .. tostring(obj.Shape)
				parts[#parts + 1] = "Material=" .. tostring(obj.Material)
				parts[#parts + 1] = "Color=" .. tostring(obj.Color)
				parts[#parts + 1] = "Transparency=" .. string.format("%.3f", obj.Transparency)
			elseif obj:IsA("Model") or obj:IsA("Folder") then
				local children = {}
				for _, child in ipairs(obj:GetDescendants()) do
					if not child:IsA("Script") and not child:IsA("LocalScript") and not child:IsA("ModuleScript") then
						children[#children + 1] = child.ClassName .. ":" .. child.Name
					end
				end
				table.sort(children)
				parts[#parts + 1] = "Children=" .. table.concat(children, "|")
			end

			return table.concat(parts, "||")
		end

		local function getClickSelectCandidate(obj)
			if not obj then return nil end
			-- Para Parts dentro de um Model, também permitimos selecionar o próprio
			-- Model quando ele for o alvo lógico disponível no Explorer.
			if nodes[cloneref(obj)] then return obj end
			return nil
		end

		local function addClickSelectHighlight(obj, index)
			if index > CLICK_SELECT_MAX_MATCHES then return false end
			local ok, isTargetable = pcall(function()
				return obj:IsA("BasePart") or obj:IsA("Model") or obj:IsA("Folder")
			end)
			if not ok or not isTargetable then return false end

			local adornee = obj
			if obj:IsA("Folder") then
				-- Folder não é suportado pelo Highlight como Adornee.
				return false
			end

			local h = Instance.new("Highlight")
			h.Name = "DexClickSelectEqual"
			h.Adornee = adornee
			...
			clickSelectHighlights[#clickSelectHighlights + 1] = h
			return true
		end

		local function highlightEquivalentObjects(target)
			clearClickSelectHighlights()
			local signature = getClickSelectSignature(target)
			if not signature then return 0 end

			local root = workspace
			local count = 0
			local descendants = root:GetDescendants()
			for i = 1, #descendants do
				if count >= CLICK_SELECT_MAX_MATCHES then break end
				local candidate = descendants[i]
				local isSame = false
				pcall(function()
					if candidate ~= target and getClickSelectSignature(candidate) == signature then
						isSame = true
					end
				end)
				if isSame and addClickSelectHighlight(candidate, count + 1) then
					count += 1
				end
			end

			-- O objeto clicado sempre recebe a mesma aura, mesmo se for o único exemplar.
			addClickSelectHighlight(target, count + 1)
			return count + 1
		end
			...
			if callback then
				...
				cptsOnMouseClick = mouse.Button1Down:Connect(function()
					pcall(function()
						...
						local target = getClickSelectCandidate(object)
						if target then
							local node = nodes[cloneref(target)]
							selection:Set(node)
							Explorer.ViewNode(node)
							highlightEquivalentObjects(target)
						end
					end)
				end)
			else
